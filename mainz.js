...
// =============================================
// LOAD QUIZ DATA (Separate Random and Topic)
// =============================================
...
async function loadFreeQuizData() {
    try {
        console.log("📥 Loading free quiz data...");
        state.quizData = { random: {}, topic: {} };
        ...
        console.log("✅ Free quiz data loaded");
    } catch (error) {
...
function generateUniqueCode() {
    ...
    return `APB${timestamp}${random}`.slice(-16);
}

function showPaymentModal() {
    closeModal(upgradeModal);
    const uniqueCode = generateUniqueCode();
    if (uniqueCodeInput) uniqueCodeInput.value = uniqueCode;
...
function copyUniqueCode() {
    if (uniqueCodeInput) {
        ...
        showToast(correctToast, "Code copied to clipboard!");
    }
}
...
async function handlePaymentSent() {
    ...
    if (!state.user || !state.user.uid) {
        showToast(incorrectToast, "Please login first to make payment");
    ...
    try {
        ...
        closeModal(paymentModal);
        showToast(correctToast, "Payment request sent! Admin will verify within 24 hours.");
        setTimeout(() => {
            alert("📱 PAYMENT INSTRUCTIONS:\n\n1. Send ₦100 to: Account: 1234567890 (Opay/Palmpay)\n\n2. Use this code in remark: " + uniqueCode + "\n\n3. Your account will be upgraded once admin confirms.");
        }, 500);
    } catch (error) {
        console.error("Error saving payment:", error);
        showToast(incorrectToast, "Error submitting payment request");
    }
}
...
function handleQuizTypeChange() {
    ...
    console.log(`🔄 Quiz type changed to: ${state.selectedQuizType}`);
...
function loadRandomQuizzes() {
    ...
    if (!quizzes || Object.keys(quizzes).length === 0) {
        ...
        noQuizzes.innerHTML = `<div class="quiz-card-header"><div class="quiz-subject">No Random Quizzes</div></div><div class="quiz-details">No quizzes available for this subject.</div>`;
    ...
    Object.entries(quizzes).forEach(([quizKey, quizData]) => {
        ...
        card.innerHTML = `
            ...
            <div class="quiz-status ${isLocked ? 'status-locked' : 'status-available'}">${isLocked ? '🔒 PREMIUM' : 'FREE'}</div></div>
...
function loadTopicQuizzes() {
    ...
    if (!topicData) {
        const noQuizzes = document.createElement('div');
        noQuizzes.className = 'quiz-card';
        noQuizzes.innerHTML = `<div class="quiz-card-header"><div class="quiz-subject">No Quizzes Available</div></div><div class="quiz-details">No quizzes available for this topic.</div>`;
...
function handleTopicQuizSelection(quizId, quizData) {
    console.log(`🎯 Selected topic quiz: ${quizId}`);
...
function initializeQuiz() {
    ...
    let title = '';
    if (state.currentQuiz.type === 'random') {
        title = state.currentQuiz.subject;
    } else {
        title = `${state.currentQuiz.subject}: ${state.currentQuiz.topic}`;
    }
...
function selectOption(optionIndex) {
    ...
    if (state.userAnswers[idx] === optionIndex) {
        ...
        showToast(isCorrect ? correctToast : incorrectToast, isCorrect ? "Correct! Check explanation." : "Incorrect. Check explanation.");
    }
    loadQuestion(idx);
    updateQuestionGrid();
}
...
function showExplanationModal(isCorrect, explanation, correctAnswer) {
    ...
    if (explanationText) explanationText.innerHTML = explanation || 'No explanation available.';
...
function finishQuiz() {
    ...
    console.log(`🎉 Quiz completed! Score: ${percent}% (${correct}/${state.questions.length})`);
}
